...
function search() {
    returnAllItems();
    if (imagesData.length === numberOfTotalImages && document.querySelector('.search__bar').value !== "") { //number of total images, is a number of img's in the container, we get that value from images_generator.js
            ...
            for (var x = 0; x < imagesData[i].tags.length; x++) {
                if (keyword === imagesData[i].tags[x]) {
                    ...
                    if (numberOfTags === 0) {
                        ...
                        var numberOfelements = numberOfelements !== null ? elements.length : 0;
...
function autocompleteBox() {
    ...
    if (imagesData.length === numberOfTotalImages) { //number of total images, is a number of img's in the container, we get that value from images_generator.js
        clearListElements();
        var keys = ''; //variable "keys" is the current input value

        document.querySelector('.search__bar').addEventListener('input', function (e) {
            document.querySelector('body').addEventListener('click', function (ev) { if (e.target !== ev.target) { keys = '' } })
            /*         console.log(e); */ /* https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/input_event */
            /*
            ...
            */
            ...
            function addDiv(i, x, tag) {
                ...
                if (document.getElementById('created' + (i + x) + tag) === null && checkIfTagAlreadyInList === 0) {
                    itemBox = document.createElement('DIV');
                    itemBox.setAttribute('id', 'created' + (i + x) + tag);
                    itemBox.setAttribute('class', 'autocomplete-items');
                    itemBox.setAttribute('onClick', 'choice("' + imagesData[i].tags[x] + '")'); //HERE I STOPTED
